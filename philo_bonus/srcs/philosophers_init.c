...
void	philosophers_init_semaphore(t_main *m, sem_t **ptr, char *name,
						unsigned int start)
{
	sem_unlink(name);
...
void	philosophers_init_semaphores(t_main *m)
{
	...
	philosophers_init_semaphore(m, &(m->sem_get_forks),
		SEMAPHORE_GET_FORKS, m->count / 2 + 1);
	philosophers_init_semaphore(m,
		&(m->sem_enough_eaten),
		SEMAPHORE_ENOUGH_EATEN, 0);
}
